...
async function saveImportedProducts() {
  ...
  await apiPost({ action: "replace_productos", productos: state.importedProducts });
  toast(`Productos guardados: ${state.importedProducts.length}`);
  await loadBootstrap();
}
...
function renderOrders() {
  const q = $("#orderFilterText").value.trim().toLowerCase();
